        ...
        <div id="sidebar" class="active">
            <div class="sidebar-wrapper active" style="width: 13vw; left:0;">
                ...
                <div style="margin-top:1rem; padding: 0 1rem;">
                    ...
                    <div class="card"
                        style="background-color: var(--bs-body-bg); width: 11vw; height: 3.5rem; margin-bottom:1rem;">
                        <div class=" btn card-body py-3 px-4" style="height: 3.5rem;" onclick="Cat(`<?= htmlspecialchars($category->get('n'), ENT_QUOTES, 'UTF-8'); ?>`)">
                            <div class="d-flex align-items-center justify-content-between">
                                <div class="ms-3 name">
                        ...
                        <?php foreach ($products as $product): ?>
                                ...
                                style="width: 7.5rem; background-color: var(--bs-body-bg);padding:0;margin:1rem;border: 1px var(--bs-body-bg) solid; ">
                                    ...
                                    <div class="card-body d-flex flex-column align-items-center" style="padding:1rem;">
                                        ...
                                        <div>
                                            <?php if ($product->get("q") > 0) {?>
                                            <button class="btn btn-sm btn-primary add-to-caisse" data-ref="<?= htmlspecialchars($product->get('r'), ENT_QUOTES, 'UTF-8'); ?>" data-lib="<?= htmlspecialchars($product->get('l'), ENT_QUOTES, 'UTF-8'); ?>"
                                            data-price="<?= htmlspecialchars($product->get('p'), ENT_QUOTES, 'UTF-8'); ?>" data-qnt="<?= htmlspecialchars($product->get('q'), ENT_QUOTES, 'UTF-8'); ?>"
                                                >Add</button>
                ...
                <div id="sidebar" class="active">
                    <div class=" sidebar-wrapper active" style="width: 31vw; right:0;">
                        <div style="margin-top:1rem; padding: 0 1rem;">
                            ...
                            <form method="post" action="<?= url_for('Presentation/caisse/confirmCaisse.php'); ?>">
                                <div class="d-flex justify-content-between">
                                    <!-- ------------ Client input -------------- -->
                                    <div>
                                        <label>Client:</label>
                                        <select name="client" type="text" value="" class=" form-select"
                                            ...
                                            <?php foreach ($clients as $client): ?>
                                                <option value="<?= htmlspecialchars($client->get('i'), ENT_QUOTES, 'UTF-8'); ?>">
                                                    <?= htmlspecialchars($client->get('n'), ENT_QUOTES, 'UTF-8'); ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
...
<script type="text/javascript">
    ...
    const orderPdfUrl = <?= json_encode(url_for('Presentation/Commande/pdf.php')); ?>;

    function print2(ref){
            const targetUrl = `${orderPdfUrl}?ref=${encodeURIComponent(ref)}`;
